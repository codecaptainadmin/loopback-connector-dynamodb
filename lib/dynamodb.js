...
DynamoDB.prototype.baseUpdate = function baseUpdate(model, id, data, condition, cb) {
  ...
  debug.enabled && debug("starting base update");
...
DynamoDB.prototype.copyParameters = function copyParameters(source, destination) {
  ...
  if(source.UpdateExpression)
    destination.UpdateExpression = source.UpdateExpression;
  if(source.ConditionExpression)
    destination.ConditionExpression = source.ConditionExpression;
  return destination;
}
