...
DynamoDB.prototype.updateDocument = function (model, entity, document, projection, cb) {
  var me = this;
  var expressions = me.computeExpression(document, projection);
  var params = {
    Key: me.getPrimaryKeyValues(model, entity),
    TableName: me.tableName(model),
    ReturnValues: "ALL_NEW"
...
DynamoDB.prototype.baseUpdate = function baseUpdate(model, id, data, condition, cb) {
  ...
  var expressions = me.computeExpression(data);
  ...
  /*if(condition) updateParams.ConditionExpression = condition;*/
  for(var exp in expressions)
...
DynamoDB.prototype.computeExpression = function computeExpression(object, projection) {
  var uvalues = {};
  var unames = {};
  var updates = [], removes = [];
