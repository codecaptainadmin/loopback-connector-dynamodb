 ...
 * @param {Function} [cb] The callback function
 */
DynamoDB.prototype.updateDocument = function (model, entity, document, properties, projection, cb) {
  var me = this;
  var expressions = computeExpression(document, properties, projection);
  var params = {
    Key: me.getPrimaryKeyValues(model, entity),
    TableName: me.tableName(model),
    ReturnValues: "ALL_NEW"
 ...
 * @param {Function} [cb] The callback function
 */
DynamoDB.prototype.scanIds = function scanIds(model, cb) {
  var me = this;
...
DynamoDB.prototype.updateAttributes = function updateAttributes(model, id, data, cb) {
  ...
  var properties = me.dataSource.definitions[model].properties;
  var expressions = computeExpression(data, properties);
...
function computeExpression(object, properties, projection) {
  var uvalues = {};
  var unames = {};
  var updates = [];
  for(var field in object) {
    var isProperty = field in properties;
    var isCollection = properties.collections && field in properties.collections;
    if(!(isProperty || isCollection)) continue;
