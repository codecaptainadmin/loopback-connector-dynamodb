...
DynamoDB.prototype.createDocument = function (model, entity, document, properties, projection, cb) {
  ...
  var params = {
    ...
    UpdateExpression: "SET " + projection + " = :document",
 ...
 * @param {Function} [cb] The callback function
 */
DynamoDB.prototype.findDocument = function(model, entity, projection, cb) {
  var me = this;
  var params = {
    Key: me.getPrimaryKeyValues(model, entity),
    TableName: me.tableName(model),
...
DynamoDB.prototype.delDocument = function(model, entity, projection, cb) {
  ...
  var params = {
    Key: me.getPrimaryKeyValues(model, entity),
    TableName: me.tableName(model),
    UpdateExpression: "REMOVE " + projection
  };
  me.client.updateItem(params, function(err, data) {
    if(err) cb(err);
    else cb(null, data.Item);
  ...
  */
  ...
  this.client.putItem(params, function(err, data) {
    if (err) {
      ...
      debug && debug(data);
      debug && debug("Item id returned");
      debug(itemToPut[hashKeyProperty]);
      debug("Create " + model + " success.");
...
DynamoDB.prototype.find = function find(model, id, cb) {
  ...
  this.client.getItem(findParams, function (err, data) {
    if (err) {
      cb(err);
    // Not sure it's necessary to create an object that isn't found - JM 2014-12-09
    /*
    } else if (!data.Item) {
      self.create(model, data, function (err, id) {
        if (err) {
          ...
          self.castDatePropertiesBackToDate(model, data);
          cb(null, data);
        }
      });
    */
...
DynamoDB.prototype.baseUpdate = function baseUpdate(model, id, data, condition, cb) {
  ...
  if(condition) {
    var conditionParams = me.parameterizeCondition(condition);

    // copy names
    if(!updateParams.ExpressionAttributeNames && conditionParams.ExpressionAttributeNamesCount) 
      updateParams.ExpressionAttributeNames = {};
    for(var field in conditionParams.ExpressionAttributeNames)
      updateParams.ExpressionAttributeNames[field] = conditionParams.ExpressionAttributeNames[field];
    
    // copy values
    if(!updateParams.ExpressionAttributeValues && conditionParams.ExpressionAttributeValuesCount) 
      updateParams.ExpressionAttributeValues = {};
    for(var field in conditionParams.ExpressionAttributeValues)
      updateParams.ExpressionAttributeValues[field] = conditionParams.ExpressionAttributeValues[field];
    
    // copy updated condition
    updateParams.ConditionExpression = conditionParams.condition;
  }

  if(id != null)
    for(var field in updateParams.Key)
      updateParams.Key[field] = id;
...
DynamoDB.prototype.getPrimaryKeyValues = function(model, data) {
  ...
  primaryKeyProperties.forEach(function (property) {
    primaryKeyValues[property.key] = data[property.key];
  });
...
DynamoDB.prototype.computeExpression = function computeExpression(model, object, projection) {
  ...
  var params = { 
    ...
    ExpressionAttributeNames: unames
  };
  if(updates.length > 0)
...
DynamoDB.prototype.parameterizeCondition = function parameterizeCondition(condition) {
  ...
  for(var r in names) {
    var expression = names[r];
    var re = new RegExp("\\b" + expression + "\\b", "g");
    condition = condition.replace(re, r);
  }
  for(var r in values) {
    var expression = values[r];
    var re;
    if(typeof expression == 'string') {
      re = new RegExp("['\"]?\\b" + expression + "\\b['\"]?", "g");
    } else {
      re = new RegExp("\\b" + expression + "\\b", "g");
    }
    condition = condition.replace(re, r);
  ...
  return {
    ...
    ExpressionAttributeNamesCount:ncount,
    ExpressionAttributeValues:values,
    ExpressionAttributeValuesCount:vcount
  };
};
